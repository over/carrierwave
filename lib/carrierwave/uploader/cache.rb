  ...
  module Uploader
    module Cache
      ...
      def cache_stored_file!
        ...
        if _content.is_a?(File) && _content.closed?
          @file = @storage.retrieve!(identifier)
          _content = file.read
        end
        
        sanitized = SanitizedFile.new(:tempfile => StringIO.new(_content), :filename => File.basename(path), :content_type => file.content_type)
        cache! sanitized
      end
